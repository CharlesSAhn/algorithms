...
def dijkstra(grapth, start_node, end_node):
    ...
    distance_dic = {node: math.inf for node in graph.nodes}

    distance_dic[start_node] = 0
    ...
    while distance_dic:
        ...
        current_node, node_distance = sorted(distance_dic.items(), key=lambda x: x[1])[0]

        # Remove the current node from the distance_dict, and store the same key:value in shortest_distance
        shortest_distance[current_node] = distance_dic.pop(current_node)
        ...
        for edge in current_node.edges:
            if edge.node in distance_dic:

                distance_to_neighbour = node_distance + edge.distance
                if distance_dic[edge.node] > distance_to_neighbour:
                    distance_dic[edge.node] = distance_to_neighbour
